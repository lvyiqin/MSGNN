...
class GraphData(object):
    ...
    def __getitem__(self, idx):
        g = self.data[idx]
        return g.A, g.feas.float(), g.label, g
    ...
    def __next__(self):
        ...
        gs, hs, labels, graphs = map(list, zip(*data))
        return len(gs), gs, hs, torch.LongTensor(labels), graphs
...
class SubgraphData(object):
    ...
    def __getitem__(self, idx):
        g = self.data[idx]
        return g.subs, g.embs, g.adjs, g.graphs, g.label
    ...
    def __next__(self):
        ...
        data = [self.__getitem__(idx) for idx in cur_idx]
        self.pos += len(cur_idx)
        gsubs, gembs, adjs, graphs, labels = map(list, zip(*data))
        return len(gsubs), gsubs, gembs, adjs, graphs, labels
